  ...
  if (state == initial)
    {
      ...
      if(chunk == 0 || m_segmentCounter == 0 || remainder == 0) {
          ...
          if (m_bDelay > 0 && m_segmentCounter < m_lastSegmentIndex)
          ...
          else
            {
		         ...
		         if(chunk == 0 || m_segmentCounter == 0 || remainder == 0) {
		            ...
		            // This is the first chunk of a new segment
                /*  e_d  */
                controllerEvent ev = delayedSend;
                Time delay = MicroSeconds(getAvailabilityTime(m_segmentCounter)-Simulator::Now ().GetMicroSeconds ());
                if(delay <= 0) {
...
TypeId TcpStreamClient::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::TcpStreamClient")
    ...
    .AddAttribute ("LogLevel",
                   ...
                   MakeUintegerChecker<uint32_t> ())
    .AddAttribute ("AbrParameter",
                   "Value to be passed on to the ABR.",
                   UintegerValue (0),
                   MakeUintegerAccessor (&TcpStreamClient::abrParameter),
                   MakeUintegerChecker<uint32_t> ())
  ;
  return tid;
}
...
void TcpStreamClient::Initialise (std::string algorithm, uint16_t clientId)
{
  ...
  else if (algorithm == "liveabr") // Llama ABR
    {
      int tempChunk = chunk;
      if(cmaf == 0) { tempChunk = 0; }
      algo = new LiveABR (m_videoData, m_playbackData, m_bufferData, m_throughput, tempChunk, cmaf, abrParameter);
    }
  else if (algorithm == "stallion")
    {
      int tempChunk = chunk;
      if(cmaf == 0) { tempChunk = 0; }
      algo = new Stallion (m_videoData, m_playbackData, m_bufferData, m_throughput, tempChunk, cmaf);
    }
  else if (algorithm == "l2a")
    {
      int tempChunk = chunk;
      if(cmaf == 0) { tempChunk = 0; }
      algo = new L2A (m_videoData, m_playbackData, m_bufferData, m_throughput, tempChunk, cmaf);
    }
  else if (algorithm == "lolplus")
    {
      int tempChunk = chunk;
      if(cmaf == 0) { tempChunk = 0; }
      algo = new LolPlus (m_videoData, m_playbackData, m_bufferData, m_throughput, tempChunk, cmaf);
    }
  ...
  while ( (packet = socket->Recv ()) )
    {
      ...
      if(chunk == 0) {
        ...
        int remainder = tcpSegmentCounter % chunk;
        if(remainder != 0) {
          if(tcpBuffer >= m_videoData.segmentSize.at (m_playbackData.playbackIndex.at(tcpSegmentCounter-remainder)).at (tcpSegmentCounter))
          {
            SegmentReceivedHandle ();
            tcpBuffer -= m_videoData.segmentSize.at (m_playbackData.playbackIndex.at(tcpSegmentCounter-remainder)).at (tcpSegmentCounter);
    	  	  tcpSegmentCounter++;
          }
        }
        else if(tcpSegmentCounter < (int)m_playbackData.playbackIndex.size()) {
          if(tcpBuffer >= m_videoData.segmentSize.at (m_playbackData.playbackIndex.at(tcpSegmentCounter)).at (tcpSegmentCounter))
          {
            SegmentReceivedHandle ();
            tcpBuffer -= m_videoData.segmentSize.at (m_playbackData.playbackIndex.at(tcpSegmentCounter)).at (tcpSegmentCounter);
    	  	  tcpSegmentCounter++;
          }
        }
...
void TcpStreamClient::SegmentReceivedHandle ()
{
  ...
  if(chunk > 0) {
	  ...
	  if(remainder != 0) {
	  	...
	  	if(m_downloadRequestSent < getAvailabilityTime(m_segmentCounter) && getAvailabilityTime(m_segmentCounter) > m_throughput.transmissionEnd.at(m_segmentCounter-1)) {

	  		m_downloadRequestSent = getAvailabilityTime(m_segmentCounter);

	  	} else {

	  		m_downloadRequestSent = m_throughput.transmissionEnd.at(m_segmentCounter-1);
...
void TcpStreamClient::LogPlayback ()
{
  ...
  /*if(chunk > 0) {
     becameAvailable = ((m_videoData.segmentDuration*chunk)*(m_currentPlaybackIndex/chunk));
  }*/
  ...
  //Update live latency
  m_playbackData.currentLiveLatency = (Simulator::Now ().GetMicroSeconds ()-(becameAvailable-m_videoData.segmentDuration))  / (double)1000000;
